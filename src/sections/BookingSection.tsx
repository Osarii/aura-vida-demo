...
function createAppointmentId() {
  return globalThis.crypto?.randomUUID?.() ?? `appointment-${Date.now()}-${Math.random()}`
}
...
export default function BookingSection({
  ...
  const times = useMemo(() => {
    const baseAvailability = getAvailability(draft.doctorId, draft.date, new Date())
  ...
  }, [appointments, draft.doctorId, draft.date, availabilityTick])

  useEffect(() => {
    const timer = window.setInterval(() => setAvailabilityTick((value) => value + 1), 60_000)
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setLoadingTimes(false), 450)
  ...
  useEffect(() => {
    ...
    if (draft.time && draft.date && draft.doctorId) setStep(4)
    else if (draft.doctorId) setStep(3)
    else if (draft.specialtyId) setStep(2)
    else setStep(1)
  }, [draft.specialtyId, draft.doctorId, draft.date, draft.time, generated])
    ...
    <section className="booking-section" id="reservar">
      <div className="container">
        ...
        <Reveal className="booking-app">
          <div className="booking-stepper" aria-label="Pasos de la reserva">
            {[
              [1, 'Especialidad'],
              [2, 'Profesional'],
              [3, 'Fecha y hora'],
              ...
              const enabled =
                n === 1 ||
                (n === 2 && Boolean(draft.specialtyId)) ||
                (n === 3 && Boolean(draft.doctorId)) ||
                (n === 4 && Boolean(draft.time))
                ...
                <div className="booking-choice-grid">
                  {specialties.map((item) => (
                    <button
                      ...
                      aria-pressed={draft.specialtyId === item.id}
                ...
                <div className="booking-doctor-grid">
                  {filteredDoctors.map((item) => (
                    <button
                      ...
                      aria-pressed={draft.doctorId === item.id}
                      key={item.id}
                      onClick={() => selectDoctor(item.id)}
                    >
                      <img src={item.photo} alt="" loading="lazy" decoding="async" onError={(event: SyntheticEvent<HTMLImageElement>) => setImageFallback(event.currentTarget, item.name)} />
                      <div>
                        <strong>{item.name}</strong>
            ...
            {step === 3 && (
              <div className="booking-step-content">
                ...
                <div className="calendar-time-layout">
                  ...
                  <div className="times-panel" aria-live="polite">
              ...
              <div className="booking-step-content confirmation-step">
                ...
                <div className="booking-summary">
                  <div className="summary-doctor">
                    <img src={doctor?.photo} alt="" loading="lazy" decoding="async" onError={(event: SyntheticEvent<HTMLImageElement>) => doctor && setImageFallback(event.currentTarget, doctor.name)} />
                    <div>
            ...
            {step === 4 && generated && doctor && specialty && (
              <div className="success-screen">
                ...
                <div className="success-ticket">
                  <div><span>Profesional</span><strong>{doctor.name}</strong></div>
                  <div><span>Especialidad</span><strong>{specialty.name}</strong></div>
                  <div><span>Fecha</span><strong>{formatDate(generated.date)}</strong></div>
                  <div><span>Hora</span><strong>{generated.time}</strong></div>
                </div>

                <div className="success-actions">
                  <button type="button" onClick={() => void downloadAppointmentPDF(generated, doctor, specialty)}>Descargar PDF</button>
